...
public class Debate_HeartRateGraphController : MonoBehaviour
{
    ...
    private int pointCount = 70; // 그래프를 구성할 점의 개수 (해상도) // 심박 다음점 위치

    [Header("Heartbeat Simulation")]
    [SerializeField] private float heartRateBPM = 160f; // 분당 심박수 (BPM) = 흘러가는 속도
    [SerializeField] private float beatSpikeHeight = 1.5f; // 심박 스파이크 높이 //  1.5 긴장상태 2 불안 5 거짓
    [SerializeField] private float beatSpikeWidth = 0.001f;  // 스파이크의 날카로운 정도 
    [SerializeField] private float noiseAmount = 0.05f; // 기본 노이즈 크기 // 기본심박 노이즈
    ...
    private void DrawGraph()
    {
        ...
        //float graphWidth = rt.rect.width;
        //float graphHeight = rt.rect.height;
        float yOffset = -rect.height / 2f; // 그래프를 중앙에 맞추기 위한 오프셋

        Vector3[] positions = new Vector3[pointCount];
        for (int i = 0; i < pointCount; i++)
        {
            float x = ((float)i / (pointCount - 1)) * rect.width;
            float y = (dataPoints[i] * (rect.height / 2f)) + yOffset; // y값을 그래프 높이에 맞게 스케일링
